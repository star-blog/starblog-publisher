    ...
    private async Task CheckForUpdate() {
        ...
        try {
            ...
            if (!response.IsSuccessStatusCode) {
                UpdateStatus = "检查更新失败";
                await GuiHost.AlertAsync("检查更新失败",
            ...
            if (!IsNewerVersion(latestVersion, ApplicationVersion.Value)) {
                ...
                await GuiHost.AlertAsync("检查更新", "当前已是最新版本。", NotificationType.Success);
                return;
            }

            UpdateStatus = $"发现新版本 {latestVersion}";
            if (await GuiHost.ConfirmAsync("发现新版本",
            ...
            UpdateStatus = "检查更新失败";
            await GuiHost.AlertAsync("检查更新失败", $"无法连接到 GitHub，请检查网络后重试。\n\n{ex.Message}", NotificationType.Error);
        }
        catch (Exception ex) {
            UpdateStatus = "检查更新失败";
            await GuiHost.AlertAsync("检查更新失败", $"检查更新时发生错误：{ex.Message}", NotificationType.Error);
        }
        finally {
